...
The four chapters police *structure*: which folder a file lives in, which layer it may import, which of view, hook or service owns a decision. Nothing in them polices what happens to a **type** once it is inside the right file.
...
Two facts made this actionable. The rules exist as a maintained set (fifteen of them, generic, no project vocabulary), and running them against this codebase produced 67 findings in application code — not a stylistic drizzle but four recurring patterns, one of them 36 instances of the same unexplained cast.
...
2. **The rule source is vendored**, at `tools/oxlint/anti-slop/`, and excluded from lint and type-check. It is a dependency that happens to live in the repo, not code we maintain.
...
**`SAFETY:` comments rot.** A rule that accepts a comment as justification accepts a *stale* comment as justification. The invariant named beside a cast in 2026 can be false by 2027 with the comment still sitting there, now actively misleading. Reviews must read them as code.

**Vendored means manually upgraded.** No `npm update` moves `tools/oxlint/anti-slop/`. It drifts from upstream silently, and re-copying it is a deliberate act somebody has to remember.
...
**Write our own smaller set.** The repo already owns [one lint plugin](../../tools/oxlint/jettison/index.ts) and that was justified because nobody ships layer rules. These rules *are* shipped. Writing a second homegrown plugin to re-derive them would be the mistake ADR-005 accepted deliberately, repeated without a reason.
