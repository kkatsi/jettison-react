...
// one part of this config that is genuinely ours. Section 5 is vendored from
// anti-slop (MIT) — see section 5 for the source, ADR-006 for the decision.
//
...
export default defineConfig({
  ignorePatterns: [
    ...
    // Upstream's code, not ours: it is upgraded by re-copying, not by editing.
  ...
  rules: {
    ...
    // ==========================================================================
    // Vendored, verbatim, from anti-slop by Dillon Mulroy (MIT) at commit 6d53855:
    // github.com/dmmulroy/anti-slop — minus its opt-in Effect rules, which this
    // repo has no use for. `tools/oxlint/anti-slop/LICENSE` travels with the copy,
    // and `cmp -r` against upstream still passes, which is why nothing in there is
    // edited. These are about the *type* a value carries, not about where the file
    // sits: an `unknown` parameter or an unexplained `as` is how a contract quietly
    // stops being one. The set is not ours and the costs of that are written
    // down — ADR-006.
